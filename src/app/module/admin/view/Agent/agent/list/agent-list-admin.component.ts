...
export class AgentListAdminComponent implements OnInit {
    ...
    plageHoraires: Array<PlageHoraireDto>;


    constructor( private service: AgentAdminService  , private plageHoraireService: PlageHoraireAdminService, private entiteAdministrativeService: EntiteAdministrativeAdminService, private posteService: PosteAdminService, @Inject(PLATFORM_ID) private platformId?) {
...
// add
    ...
    public initCol() {
        this.cols = [
            ...
            {field: 'nomUtilisateur', header: 'Nom utilisateur'},
            {field: 'motDePasse', header: 'Mot de passe'},
            {field: 'nom', header: 'Nom'},
            {field: 'prenom', header: 'Prenom'},
            {field: 'adresse', header: 'Adresse'},
            {field: 'entiteAdministrative?.libelle', header: 'Entite administrative'},
            {field: 'poste?.libelle', header: 'Poste'},
            {field: 'plageHoraire?.ref', header: 'Plage horaire'},
        ];
    }
    ...
    public async loadPoste(){
        this.posteService.findAllOptimized().subscribe(postes => this.postes = postes, error => console.log(error))
    }
    public async loadPlageHoraire(){
        this.plageHoraireService.findAllOptimized().subscribe(plageHoraires => this.plageHoraires = plageHoraires, error => console.log(error))
    }
   ...
   public prepareColumnExport(): void {
        this.exportData = this.items.map(e => {
            return {
                 'Ref': e.ref ,
                 'Nom utilisateur': e.nomUtilisateur ,
                 'Mot de passe': e.motDePasse ,
                 'Nom': e.nom ,
                 'Prenom': e.prenom ,
                 'Adresse': e.adresse ,
                'Entite administrative': e.entiteAdministrative?.libelle ,
                'Poste': e.poste?.libelle ,
                'Plage horaire': e.plageHoraire?.ref ,
            }
        });

        this.criteriaData = [{
            'Ref': this.criteria.ref ? this.criteria.ref : environment.emptyForExport ,
            'Nom utilisateur': this.criteria.nomUtilisateur ? this.criteria.nomUtilisateur : environment.emptyForExport ,
            'Mot de passe': this.criteria.motDePasse ? this.criteria.motDePasse : environment.emptyForExport ,
        ...
        //'Poste': this.criteria.poste?.libelle ? this.criteria.poste?.libelle : environment.emptyForExport ,
        //'Plage horaire': this.criteria.plageHoraire?.ref ? this.criteria.plageHoraire?.ref : environment.emptyForExport ,
        }];
      }
